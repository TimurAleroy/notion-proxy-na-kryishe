...
app.post('/api/booking', async (req, res) => {
  const { name, phone: rawPhone, date, time, guests, comment, room } = req.body;
  ...
  try {
    ...
    // Отправляем уведомление управляющему напрямую через бота — надёжнее диплинка
    const titleLine = room ? '📅 Новая бронь VIP-комнаты!' : '📅 Новая бронь с сайта!';
...
app.post('/api/profile', async (req, res) => {
  const { name, username, phone: rawPhone, birthday } = req.body;
  const phone = normalizePhone(rawPhone);
