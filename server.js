...
app.post('/api/booking', async (req, res) => {
  ...
  try {
    ...
    // Гостю — подтверждение приёма брони (только если бот может ему написать)
    if (telegramId) {
    ...
    // Администратору — уведомление с возможностью связаться с гостем
    let contactLine;
    if (telegramUsername) {
      contactLine = `\n\n💬 Написать гостю: https://t.me/${telegramUsername}`;
    } else if (telegramId) {
      contactLine = `\n\n📩 Гость получил подтверждение от бота (у него нет username — прямую ссылку дать нельзя, но бот ему написал)`;
    } else {
...
app.post('/api/booking/cancel', async (req, res) => {
  const { phone: rawPhone, entry, name } = req.body;
  const phone = normalizePhone(rawPhone);
  ...
  try {
    ...
    const guestName = props['Имя']?.title?.[0]?.plain_text || name || 'Гость';
    const telegramId = props['Telegram ID']?.rich_text?.[0]?.plain_text;
    const telegramUsername = props['Telegram Username']?.rich_text?.[0]?.plain_text;

    if (telegramId) {
      await sendTelegramMessage(
        telegramId,
        `Бронирование отменено. Будем рады видеть вас в следующий раз.\n\n🗓 ${entry}`
      );
    }

    const contactLine = telegramUsername ? `\n💬 https://t.me/${telegramUsername}` : '';
    await sendTelegramMessage(
      ADMIN_CHAT_ID,
      `❌ Гость отменил бронь\n\n👤 ${guestName}\n📱 ${phone}\n🗓 ${entry}${contactLine}`
    );
