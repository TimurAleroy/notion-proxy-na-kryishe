...
// Каждая запись в "Истории броней" хранится как "ISO-дата|человекочитаемый текст" (плюс статус в конце).
// Это позволяет автоматически понимать, что бронь уже прошла.
function splitEntry(raw) {
  const idx = raw.indexOf('|');
  return idx === -1 ? { iso: null, text: raw } : { iso: raw.slice(0, idx), text: raw.slice(idx + 1) };
}
function displayEntry(raw) {
  return splitEntry(raw).text;
}
...
app.post('/api/booking', async (req, res) => {
  ...
  try {
    ...
    const displayText = date ? `${date}${time ? ' ' + time : ''}` : new Date().toISOString().split('T')[0];
    const isoTime = time && /^\d{2}:\d{2}$/.test(time) ? time : '00:00';
    const isoDateTime = dateISO ? `${dateISO}T${isoTime}:00` : null;
    const bookingEntry = isoDateTime ? `${isoDateTime}|${displayText}` : displayText;
...
app.get('/api/booking/status', async (req, res) => {
  ...
  try {
    ...
    // Если дата брони уже прошла — считаем что бронь завершена, статус сбрасывается
    const { iso } = splitEntry(latestActive);
    if (isEntryExpired(iso)) {
      return res.json({ status: 'none' });
    }

    const raw = baseEntry(latestActive); // без статус-суффикса — нужен для отмены/опоздания
    const display = displayEntry(raw); // без ISO-префикса — для показа гостю
